...
export const getProducts =
    (keyword = "", category, price = [0, 200000], ratings = 0, currentPage = 1) => async (dispatch) => {
        try {
            ...
            let url = `${backend_url}/api/v1/products?keyword=${keyword}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}&page=${currentPage}`;
            if (category) {
                url = `${backend_url}/api/v1/products?keyword=${keyword}&category=${category}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}&page=${currentPage}`;
            }
...
export const getSimilarProducts = (category) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${backend_url}/api/v1/products?category=${category}`);
...
export const getSliderProducts = () => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${backend_url}/api/v1/products/all`);
...
export const getAllReviews = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.get(`${backend_url}/api/v1/admin/reviews?id=${id}`);
...
export const deleteReview = (reviewId, productId) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.delete(`${backend_url}/api/v1/admin/reviews?id=${reviewId}&productId=${productId}`);
